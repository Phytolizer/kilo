...
#define APPEND_BUFFER_INIT \
  (struct AppendBuffer) { .buffer = NULL, .length = 0, }
...
void Editor_Scroll() {
  ...
  if (g_editorConfig.cursorY >=
      g_editorConfig.rowOffset + g_editorConfig.screenRows) {
    g_editorConfig.rowOffset =
        g_editorConfig.cursorY - g_editorConfig.screenRows + 1;
  }
  if (g_editorConfig.cursorX < g_editorConfig.colOffset) {
    g_editorConfig.colOffset = g_editorConfig.cursorX;
  }
  if (g_editorConfig.cursorX >=
      g_editorConfig.colOffset + g_editorConfig.screenCols) {
    g_editorConfig.colOffset =
...
void Editor_RefreshScreen() {
  ...
  snprintf(buf, sizeof(buf), "\x1b[%d;%dH",
           g_editorConfig.cursorY - g_editorConfig.rowOffset + 1,
...
void Editor_MoveCursor(int key) {
  EditorRow* row = (g_editorConfig.cursorY >= g_editorConfig.numRows)
                       ? NULL
                       : &g_editorConfig.row[g_editorConfig.cursorY];
  switch (key) {
    case ARROW_LEFT:
      if (g_editorConfig.cursorX > 0) {
        ...
        g_editorConfig.cursorX =
            g_editorConfig.row[g_editorConfig.cursorY].size;
      }
      break;
    case ARROW_RIGHT:
  ...
  row = (g_editorConfig.cursorY >= g_editorConfig.numRows)
            ? NULL
            : &g_editorConfig.row[g_editorConfig.cursorY];
...
void Editor_Init() {
  ...
  if (GetWindowSize(&g_editorConfig.screenRows, &g_editorConfig.screenCols) ==
      -1) {
    Die("GetWindowSize");
  }
}
